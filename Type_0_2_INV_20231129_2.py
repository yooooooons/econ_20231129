...
def type_0_buy_sell_normal (target_dic) :
    ...
    if target_dic['bought_state'] == 1 :   # 매수가 되어 있는 상태라면
        ...
        if ((DF_check_peak['low'][-1] / DF_check_peak.loc[DF_check_peak.iloc[-(time_elapse_bought + 1) : -1]['high'].idxmax()]['high']) < (1 - target_dic['ratio_sell_forced_peak'])) and         (DF_check_peak.iloc[-4 : -1]['volume'].maen() > (5 * DF_check_peak.iloc[-51 : -1]['volume'].mean())) :
